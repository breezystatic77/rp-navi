...
function createAccount(username, password, email) {
	...
	let accountQuery = accountModel
		.findOne({ usernameLower: usernameLower })
		.exec()
...
function getAccountData(
	username,
	searchQuery = ['-username', 'username', 'createdAt']
) {
	//'username email blocked verification.verified'
	let usernameLower = username.toLowerCase()
	let searchString = searchQuery.join(' ')
	let accountQuery = accountModel
		.findOne({ usernameLower: usernameLower }, searchString)
		.exec()
...
function verifyAccount(username, verifyKey) {
	...
	return dbQuery
		.then(document => {
			logger.debug(
				'%s: Comparing submitted key %s to stored key %s',
				verifyAccount.name,
				verifyKey,
				document.verifyKey
			)
...
function verifyPasswordReset(resetKey) {
	...
	logger.info(
		'%s: Someone is checking if key %s exists',
		verifyPasswordReset.name,
		resetKey
	)
	return accountQuery
		.then(accountDoc => {
			if (accountDoc && checkIfExpired(accountDoc.resetPwExpire.getTime())) {
				logger.info(
					'Key %s has turned up with user %s',
					resetKey,
					accountDoc.username
				)
